...
export default {
	...
	methods: {
		get_pdf_url(type = 'both') {
			let _items = this.item.items.map(el => el.shipStation_order_id);
			let _url = new URL(window.Stackonet.ajaxurl),
				params = _url.searchParams;
			params.set('action', 'yousaidit_download_pdf');
			params.set('type', type);
			params.set('card_size', this.item.card_size);
			params.set('card_width', this.item.width);
			params.set('card_height', this.item.height);
			params.set('inner_message', this.item.inner_message);
			params.set('card_type', this.item.card_type);
				...
				.then((confirmed) => {
					if (confirmed) {
						...
						item.to_generate.forEach(_item => {
							this.generate_dynamic_pdf(_item.wc_order_id, _item.wc_order_item_id).then(() => {
