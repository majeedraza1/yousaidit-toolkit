<template>
  <div :class="`shadow-lg card-canvas card-canvas--${card_size}`" :style="canvas_styles">
    ...
    >
      <span v-if="section.section_type === 'static-text'">
        {{ section.text }}
      </span>
      <span v-if="section.section_type === 'input-text'">
        {{ section.placeholder }}
      </span>
  ...
  methods: {
    ...
    sectionStyle(section) {
      ...
      if (section.section_type === 'static-text' || section.section_type === 'input-text') {
        ...
        if (section.textOptions.rotation) {
          styles.push({'--transform': `${section.textOptions.rotation}deg`})
        } else {
          styles.push({'--transform': 'none'})
        }
        if (section.textOptions.spacing) {
          styles.push({'--spacing': `${section.textOptions.spacing}pt`})
        } else {
          styles.push({'--spacing': 0})
        }
      }
      return styles
    },
...
.section-type--static-text > span,
.section-type--input-text > span {
