...
export default {
  ...
  mounted() {
    ...
    document.addEventListener('click', event => {
      const form = event.target.closest('.card-popup-form')
      if (event.target.closest('.card-popup-add-a-message') && form) {
        event.preventDefault();
        this.product_id = parseInt(form.querySelector('[name="product_id"]').value);
        this.card_size = form.querySelector('[name="attribute_pa_size"]').value;
        this.showModal = true;
        this.isCardCategoryPopup = true;
      }
    });
  },
  methods: {
    ...
    onUpdateMultiCompose(data) {
      ...
      if (this.isCardCategoryPopup) {
        return this.onUpdatePopupItemInfo(data);
      }
