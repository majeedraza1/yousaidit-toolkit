...
watch(() => props.modelValue, newValue => state.password = newValue);
watch(() => state.password, newValue => emit("update:modelValue", newValue));

onMounted(() => {
  state.password = props.modelValue;
})
</script>

<template>
  <div class="relative">
    <input :type="state.showPassword ? 'text' : 'password'" :autocomplete="props.autocomplete"
           :placeholder="props.placeholder" v-model="state.password"
