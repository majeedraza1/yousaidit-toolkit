...
import SingleProductDynamicCard from "../frontend-inner-message/SingleProductDynamicCard.vue";
import {closeModal, refreshBodyClass} from "./modal.ts";
...
const dynamicCardContainer = (product_id: number | string, card_size: string): Promise<HTMLElement> => {
  return new Promise(resolve => {
    let el = document.querySelector<HTMLElement>('#dynamic-card-container');
    if (el) {
      resolve(el);
    } else {
      el = createEl('div',
...
const submitFormToServer = (data: DynamicCardPropsInterface) => {
  ...
  for (const [key, value] of Object.entries(data.right)) {
    ...
    const inputEl = form.querySelector<HTMLInputElement>(idAttribute);
    if (inputEl) {
      inputEl.value = value;
    }
  }
...
document.body.addEventListener('update.CardCategoryPopup', (event: CustomEvent) => {
  const data = event.detail as DynamicCardPropsInterface;
