...
import {
  ...
  ShaplaFileUploader as FileUploader,
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      this.errors = {};
      Spinner.show();
      axios.post('designers/' + this.designer_id + '/cards', this.card).then(response => {
        this.closeModal();
        Spinner.hide();
        Notify.success('Card has been submitted successfully.', 'Success!');
        this.card = {
    ...
    handleCardImageFailed(fileObject, serverResponse) {
      if (serverResponse.message) {
        Notify.error(serverResponse.message);
      }
    },
