...
const card_dimension = computed<number[]>(() => {
...
const calculateWidthAndHeight = () => {
  ...
  if (document.body.offsetWidth < 1024) {
    ...
    state.height = Math.round(state.width * (card_dimension[1] / card_dimension[0]));
  } else {
    state.height = innerEL.offsetHeight;
    state.width = Math.round(innerEL.offsetHeight * (card_dimension[0] / card_dimension[1]));
  }
}
