let isCheckoutShippingFieldsValid = () => {
  ...
  requiredFields.forEach(field => {
    let _input = field.querySelector<HTMLInputElement | HTMLSelectElement>('input, select');
    if ('' === _input.value) {
      isValid = false;
    }
  });

  let sent_to = document.querySelector<HTMLInputElement>('[name="item_sent_to"]:checked');
...
let getTabNav = function (indexNumber: number) {
...
let activateTab = function (indexNumber: number) {
  ...
  let tabs = document.querySelectorAll<HTMLDivElement>('[data-tab]');
...
document.addEventListener('click', event => {
  let navToActivate = (event.target as HTMLElement).closest('.checkout-tab-item'),
    notificationDiv = document.querySelector<HTMLDivElement>('.checkout-notification');
...
document.addEventListener('click', event => {
  if ((event.target as HTMLElement).classList.contains('radio-item_sent_to')) {
    let sent_to = document.querySelector<HTMLElement>('.field--checkout_shipping');
...
document.addEventListener('click', event => {
  let navClicked = '';
  if ((event.target as HTMLElement).closest('.button--checkout-pre')) {
    event.preventDefault();
    navClicked = 'pre';
  }
  if ((event.target as HTMLElement).closest('.button--checkout-next')) {
    event.preventDefault();
    navClicked = 'next';
  }
...
document.addEventListener('click', event => {
  let navToActivate = (event.target as HTMLElement).closest('.checkout-inline-tabs__item');
  ...
  let tabs = document.querySelectorAll<HTMLElement>('[data-inline_tab]');
  tabs.forEach((_tab) => {
