...
export default {
	...
	methods: {
		...
		updateFromMessageLines(lines) {
			...
			setTimeout(() => this.$emit('input', contentEl.innerHTML), 100)
		}
	},
	mounted() {
		...
		setTimeout(() => {
			...
			this.editableContentEditor.addEventListener('paste', event => {
				...
				window.console.log(contentEl.innerHTML);
		...
		document.addEventListener('openAiContent', (event) => {
			if (this.openAiEditable) {
				this.updateFromMessageLines(event.detail.lines);
			}
		})
	}
}
